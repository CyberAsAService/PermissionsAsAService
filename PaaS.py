...
app = Flask(__name__)
app.config.update(
    CELERY_BROKER_URL=f'redis://{os.environ["REDIS"]}:6379/1',
    CELERY_RESULT_BACKEND=f'redis://{os.environ["REDIS"]}:6379/1'
)
...
@celery.task
def update(result):
    req = requests.patch(f'{os.environ["UPDATE_ADDRESS"]}', data=result)
...
def PaaS():
    ...
    if not request.json or 'address' not in request.json or 'username' not in request.json or 'steps' not in request.json:
        abort(400)
    task = _PaaS.apply_async(args=(request.json['steps'], request.json['address'], request.json['username']),
                             link=update.s())
